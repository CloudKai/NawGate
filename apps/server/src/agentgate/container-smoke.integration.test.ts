...
describe("AgentGate real-container smoke", () => {
  it.skipIf(process.env.RUN_CONTAINER_SMOKE !== "1")(
    ...
    async () => {
      ...
      const root = await mkdtemp(path.join(tmpdir(), "agentgate-container-smoke-"));
      ...
      const run = async (...args: string[]) => execFileAsync(
        ...
        [
          ...
          "--env", "AGENTGATE_RUNTIME_TOKEN",
          "--env", "AGENTGATE_GATEWAY_URL",
          IMAGE,
          "agentctl",
          ...args,
        ],
        {
          env: {
            ...process.env,
            AGENTGATE_RUNTIME_TOKEN: credential.token,

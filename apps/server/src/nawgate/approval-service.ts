...
import {
  NAWGATE_POLICY_REVISION,
  NAWGATE_POLICY_VERSION,
  NAWGATE_RISK_VERSION,
  type ApprovalRecord,
...
function normalizeBinding(request: ApprovalRequest): NormalizedBinding | null {
  ...
  const policyRevision = request.policyRevision ?? NAWGATE_POLICY_REVISION;
  ...
  const riskVersion = request.riskVersion ?? NAWGATE_RISK_VERSION;
...
export class ApprovalService {
  ...
  async approve(approvalId: string, humanId: HumanId): Promise<{ approval: ApprovalRecord; capability: CapabilityLease | null }> {
    ...
    await this.audit.record({
      ...
      policyVersion: NAWGATE_POLICY_VERSION, explanation: outcome.capability
    ...
    if (outcome.capability) {
      await this.audit.record({
        ...
        policyVersion: NAWGATE_POLICY_VERSION, explanation: "A durable one-use capability claim was issued for the exact approved action scope.",
  ...
  async deny(approvalId: string, humanId: HumanId): Promise<ApprovalRecord> {
    ...
    await this.audit.record({
      ...
      policyVersion: NAWGATE_POLICY_VERSION, explanation: "An eligible human denied this protected action before execution.",
  ...
  async consumeCapability(request: ApprovalRequest & { approvalId: string }): Promise<CapabilityConsumption> {
    ...
    if (outcome.status === "consumed") {
      await this.audit.record({
        ...
        approvalId: outcome.capability.approvalId, capabilityId: outcome.capability.id, status: "success", durationMs: null,
        policyVersion: NAWGATE_POLICY_VERSION, explanation: "The one-use capability claim was consumed for its exact bound request.",
        enforcementPoint: "ApprovalService", protectedActionExecuted: false, ...evidence(outcome.capability),
      });
    }
    return outcome;
  ...
  private async recordApprovalExpired(approval: ApprovalRecord): Promise<void> {
    await this.audit.record({
      ...
      policyVersion: NAWGATE_POLICY_VERSION, explanation: "The approval window expired before the protected action executed.",
      enforcementPoint: "ApprovalService", protectedActionExecuted: false, ...evidence(approval),
    });
  }
}

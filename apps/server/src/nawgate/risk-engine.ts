...
import { NAWGATE_RISK_VERSION } from "./types.js";
...
export type RiskAssessment =
  | {
      outcome: "allow";
      riskTier: RiskTier;
      riskVersion: typeof NAWGATE_RISK_VERSION;
      facts: RiskFacts;
      factsDigest: string;
    }
  | {
...
const ACTIONS = new Set<NawGateAction>([
  "resource.read",
  "file.read",
  "deploy.staging",
  "deploy.production",
...
function isRiskFacts(value: unknown): value is RiskFacts {
  ...
  return (
    typeof value.action === "string" && ACTIONS.has(value.action as NawGateAction) &&
  ...
  assess(input: unknown): RiskAssessment {
    if (!isRiskFacts(input)) {
      return {
        outcome: "deny",
        riskTier: "critical",
        riskVersion: NAWGATE_RISK_VERSION,
        facts: null,
        factsDigest: null,
        reasonCode: "risk_facts_malformed",
      };

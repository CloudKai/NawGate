...
const actions: readonly NawGateAction[] = [
  "resource.read",
  "file.read",
  "deploy.staging",
  "deploy.production",
...
function isKnownAction(value: unknown): value is NawGateAction {
  return typeof value === "string" && actions.includes(value as NawGateAction);
}
...
function contentDecision(input: PolicyInput): PolicyDecision {
  ...
  const action = input.action.name as NawGateAction;

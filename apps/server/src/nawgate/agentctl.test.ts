...
function runCli(
  ...
  const bootstrap = `
globalThis.__nawgateReceived = [];
let actionCount = 0;
globalThis.fetch = async (url, options = {}) => {
  const pathname = new URL(url).pathname;
  const headers = options.headers ?? {};
  const body = options.body ? JSON.parse(options.body) : null;
  globalThis.__nawgateReceived.push({ pathname, headers, body });
...
process.argv = ["node", "agentctl", ...JSON.parse(process.env.NAWGATE_TEST_ARGS)];
await import(${JSON.stringify(cliPath)});
process.stdout.write("\\n__NAWGATE_RECEIVED__" + JSON.stringify(globalThis.__nawgateReceived));
`;
  return new Promise((resolve) => {
    const child = spawn(process.execPath, ["--input-type=module", "--eval", bootstrap], {
      env: {
        ...process.env,
        NAWGATE_GATEWAY_URL: "http://nawgate.test",
        NAWGATE_RUNTIME_TOKEN: token,
        NAWGATE_APPROVAL_WAIT_MS: waitMs,
        NAWGATE_TEST_ARGS: JSON.stringify(args),
      },
    ...
    child.on("close", (code) => {
      const marker = "__NAWGATE_RECEIVED__";
...
describe("agentctl", () => {
  it("uses the narrow resource command and never sends browser auth", async () => {
    ...
    expect(result.stdout).toContain("NawGate: resource.read project-a -> ALLOW");
    ...
    expect(result.received[0]).toMatchObject({
      ...
      headers: { "X-NawGate-Runtime": "TEST_RUNTIME_TOKEN_DO_NOT_PRINT" },
  ...
  it("adds only the narrow file.read command for protected team files", async () => {
    ...
    expect(result.code).toBe(0);
    expect(result.stdout).toContain("NawGate: file.read team-alpha-internal -> ALLOW");
    expect(result.received).toHaveLength(1);
    expect(result.received[0]).toMatchObject({
      pathname: "/api/runtime/actions",
  ...
  it("polls approval with bounded retries and reuses the request id exactly once", async () => {
    ...
    expect(result.stdout).toContain("NawGate: owner approved once -> ALLOW");
